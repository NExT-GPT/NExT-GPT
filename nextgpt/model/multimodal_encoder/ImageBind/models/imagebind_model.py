    ...
    def forward(self, inputs):
        ...
        for modality_key, modality_value in inputs.items():
            ...
            if modality_value is not None:
                ...
                # print('0000', modality_value["trunk"]["tokens"].shape)  # image: torch.Size([B, 257, 1280]) audio: torch.Size([B*S, 229, 768]) video: torch.Size([B*S, 257, 1280])
                trunk_inputs = modality_value["trunk"]
                head_inputs = modality_value["head"]
                modality_value = self.modality_trunks[modality_key](**trunk_inputs)
                # print('0000', modality_value.shape)
                modality_value = self.modality_heads[modality_key](
                    modality_value, **head_inputs
                )
                # print('111', modality_value.shape) # image: torch.Size([B, 1024]) audio: torch.Size([B*S, 1024]) video: torch.Size([B*S, 1024])
                ...
                dim = modality_value.shape[-1]
                if reduce_list:
                    modality_value = modality_value.reshape(B, S, -1)
                    modality_value = modality_value.mean(dim=1)
                modality_value = modality_value.reshape(B, -1, dim)
