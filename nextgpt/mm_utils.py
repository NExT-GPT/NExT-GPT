...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    return torch.stack(image_patches, axis=0)
...
def tokenizer_multiple_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, video_token_index=VIDEO_TOKEN_INDEX, audio_token_index=AUDIO_TOKEN_INDEX, return_tensors=None):

    input_ids = []
    vid_chunks = prompt.split("<video>")
    for vid_idx, vid_ck in enumerate(vid_chunks):
        aud_chunks = vid_ck.split("<audio>")
        for aud_idx, aud_ck in enumerate(aud_chunks):
            _inputs = tokenizer_image_token(aud_ck, tokenizer, image_token_index, return_tensors=return_tensors)
            input_ids.extend(_inputs)
            if aud_idx < len(aud_chunks) - 1:
                input_ids.append(audio_token_index)
        if vid_idx < len(vid_chunks) - 1:
...
class KeywordsStoppingCriteria(StoppingCriteria):
    def __init__(self, keywords, tokenizer, input_ids):
        ...
        for keyword in keywords:
            ...
            self.keyword_ids.append(torch.tensor(data=cur_keyword_ids))
        self.tokenizer = tokenizer
        self.start_len = input_ids.shape[1]

    def call_for_batch(self, output_ids: torch.Tensor, scores: torch.Tensor, **kwargs) -> bool:
        offset = min(output_ids.shape[1] - self.start_len, self.max_keyword_len)
        self.keyword_ids = [keyword_id for keyword_id in self.keyword_ids]
        for keyword_id in self.keyword_ids:
            truncated_output_ids = output_ids[0, -keyword_id.shape[0] :]
            if torch.equal(truncated_output_ids, keyword_id).astype("bool").all():
                return True
    ...
    def __call__(self, output_ids: torch.Tensor, scores: torch.Tensor, **kwargs) -> bool:
        outputs = []
...
def preprocess_llama_2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(target.not_equal(y=torch.tensor(tokenizer.pad_token_id)).sum())
...
def get_conversation(
    version: Sequence[str], sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False
) -> Dict:
    """
